...
def get_all_customers(csd_host: str,
                      username: str,
                      password: str,
    ...
    page 153, API Connectivity Services Director API Reference, version 5.2R1
    Chapter: Customer Management RESTful Web Services
    Section: Get All Customers (Version 2)
    ...
    headers_accept_json = ('application/vnd.net.juniper.space.customer-management.'
                           + 'customers+json;version=2')
    # headers_accept_xml = ('application/vnd.net.juniper.space.customer-management.'
    #                        + 'customers+xml;version=2')
    # point to JSON or XML to select response format
    header_accept = headers_accept_json
...
def example_of_use():
    ...
    try:
        # issue the query
        response = get_all_customers(host, username, password, debug_level)
        # response = get_all_customers(host, username, password)
