...
def get_all_pe_devices(csd_host: str,
                       username: str,
                       password: str,
                       debug_level: str = 'WARNING') -> dict:
    """Retrieve all PE devices in CSD.

    API request built as per information in:
    page 178, API Connectivity Services Director API Reference, version 5.2R1
    Chapter: Provider Edge Device Management RESTful Web Services
    Section: Get All PE Devices
    ...
    header_accept_json = 'application/vnd.net.juniper.space.device-roles.pe-devices+json'
    # header_accept_xml = 'application/vnd.net.juniper.space.device-roles.pe-devices+xml'
...
def example_of_use():
    ...
    try:
        # issue the query
        response = get_all_pe_devices(host, username, password, debug_level)
