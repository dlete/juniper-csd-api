...
def ip_service_order_delete(csd_host: str,
                            username: str,
                            password: str,
                            order_id: str,
                            debug_level: str = 'WARNING') -> dict:
    """Delete a particular Service Order from a Juniper CSD system

    API request built as per information in:
    page 1744 of API Connectivity Services Director API Reference, version 5.2R1
    Chapter: IP Service Orders Management RESTful Web Services
    Section: Delete an IP Service Order
    ...
    Required:
        ...
        order_id (str):     service order id in CSD
    Optional:
    ...
    url = scheme + host + uri + order_id

    # Compose HTML headers to send with the request
    # CURRENTTLY, OCTOBER 2020, CSD 5.2 DOES NOT FAILS WHEN SETTING THE Accept HEADER -> BUG!!!
    # header_accept_json = 'application/vnd.net.juniper.space.service-management.service-order+json'
    # header_accept_xml = 'application/vnd.net.juniper.space.service-management.service-order+xml'
    ...
    try:
        ...
        r = requests.delete(
            url = url,
            # headers = headers_dict,       # see bug above
    ...
    #
    # SEE BUG ON ACCEPT HEADERS -> NEED TO USE r.text
    request_output = {
...
def example_of_use():
    ...
    try:
        # issue the query
        response = ip_service_order_delete(host, username, password, order_id, debug_level)
        # response = ip_service_order_delete(host, username, password, order_id)
