...
def get_an_ip_service_by_service_id(csd_host: str,
                                    username: str,
                                    password: str,
                                    service_id: str,
                                    debug_level: str = 'WARNING') -> dict:
    """Retrieve the details of a particular IP service from a Juniper CSD system

    API request built as per information in:
    page 1824, API Connectivity Services Director API Reference, version 5.2R1
    Chapter: IP Services Management RESTful Web Services
    Section: Get an IP Service By Service ID
    ...
    headers_dict = {        # uncomment one or the other to receive output in JSON or XML
...
def example_of_use():
    ...
    try:
        # issue the query
        response = get_an_ip_service_by_service_id(host,
                                                   username,
                                                   password,
                                                   service_id,
                                                   debug_level)
        # response = get_an_ip_service_by_service_id(host, username, password, service_id)
