...
def csd_service_order_deploy_or_validate(csd_host: str,
                                         username: str,
                                         password: str,
                                         input_values: dict,
                                         debug_level: str = 'WARNING') -> dict:
    """Deploy or Validate a CSD Service Order

    API request built as per information in:
    page 1743 of API Connectivity Services Director API Reference, version 5.2R1
    Chapter: IP Service Orders Management RESTful Web Services
    Section: Deploy, Validate, and Partially-Delete an IP Service Order
    ...
    Required:
        ...
        input_values (dict):    dictionary with all the values necessary
                                to validate the CSD Service Order
    Optional:
    ...
    input_values = {
        'service_order_name': 'so_1005_1652',
        'service_order_id': '32604389',
    ...
    To-do:
        * amalgamate into consolidated and generic API request
    ...
    url = scheme + host + uri + service_order_id

    # Compose HTML headers to send with the request
    headers_dict = {
        # 'Accept': 'n/a',                        # THIS API ENDPOINT DOES NOT ACCEPT "Accept"
    ...
    from build_csd_api_request_service_order_deploy_or_validate \
        import build_csd_api_requests_service_order_deploy_or_validate
    body_content = build_csd_api_requests_service_order_deploy_or_validate(input_values)
    ...
    try:
        ...
        r = requests.post(                              # method is POST
            url = url,                                  # API endpoint
            headers = headers_dict,                     # HTML headers
            data = body_content,                        # body of the request, payload
            auth = (username, password),                # authentication credentials
            verify = verify_host_ssl,                   # do not verify host SSL certificate
        )
...
def example_of_use():
    ...
    input_values = {
        'service_order_name': 'so_1005_1652',                       # does NOT have effect
        'service_order_id': '32604390',                             # does YES have effect
        'service_order_action': 'ValidateNow',                      # does YES have effect
        'service_order_schedule': '2019-12-11T13:57:28.000Z',       # does NOT have effect
    }

    # The options must be specified in the Action Element inside
    # OperationSpecificData. The various options are as follows:
    # * DeployNow (Deploy an already created SO)
    # * DeployLater (Deploy Later an already created SO)
    # * ValidateNow (Validate now an already created SO)
    # * ValidateLater (Validate later an already created SO)
    ...
    # issue the query
    response = csd_service_order_deploy_or_validate(host,
                                                    username,
                                                    password,
                                                    input_values,
                                                    debug_level)
    # response = csd_service_order_deploy_or_validate(host, username, password, input_values)
    pprint(response)
